    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            ...
            if 0 <= col < 8 and 0 <= ligne < 8:
                ...
                # CAS 1 : Rien n'est sélectionné -> On essaie de sélectionner
                if self.selection is None:
                    if contenu_case is not None:
                        # On vérifie que la pièce appartient au joueur dont c'est le tour
                        if contenu_case[0] == self.tour:
                            self.selection = (ligne, col)
                
                # CAS 2 : Une pièce est déjà sélectionnée -> On essaie de bouger
                else:
    ...
    def draw(self):
        ...
        for ligne in range(8):
            for col in range(8):
                ...
                if code_piece:
                    couleur_p = code_piece[0]
                    type_p = code_piece[1]
                    col_pixel = 7 if couleur_p == 'w' else 0
                    
                    pixels = self.modeles_pieces[self.map_noms[type_p]]
                    for px, py in pixels:
